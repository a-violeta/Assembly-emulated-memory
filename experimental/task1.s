...
indexare:			#foloseste eax, ebx, edx
	...
	mov %edx, j		#posibil nenecesar, eax si edx oricum intorceau valorile
	mov %eax, i
indexare_end:
	pop %ebp
	ret
...
redimensionare_ADD:
	...
	jle et_spatiu_insuficient_ADD	#dim neconforma
        ...
        mov %eax, %ebx          #dim (in blocuri) e acum si in ebx si in dim
        mov %eax, dim
...
DELETE:
       	push %ebp
        mov %esp, %ebp
        movb 8(%ebp), %bl               #fd, oare ramane aici dupa ce efectuez procese?
        mov $memorie, %esi
...
et_afisare_DEL:				#un else la cmp uri
	...
	push fd
	call GET			#acum am start si end, indecsi intregi
	add $4, %esp

	push end
	call indexare
	add $4, %esp
	push j				#reg cum sunt dupa asta? functioneaza
...
et_iterare_DEF:
	cmp $1048576, %ecx                   #DIMENSIUNE
        ...
        movb (%esi, %ecx, 1), %dl	#v[i]
        movb %dl, fd                    #nenecesar cred, dar merge si asa
...
#et_open_pro_fis:
#	mov $5, %eax                    #sys open
#        mov path_fis, %ebx              #adresa, posibil sa fie nevoie de $
#        mov $0, %ecx                    #read only
#        int $0x80

#        cmp $0, %eax
#        jl et_error_procesare_fis

#        mov %eax, %edi                  #fd
#        mov %eax, fd                    #??????
#et_lseek_pro_fis:
#        mov $19, %eax                   #sys lseek
#        mov %edi, %ebx                  #fd
...
#        mov %eax, dim                   #dim
#et_fd_pro_fis:
#        mov %edi, %eax                  #converteste fd, edi mai are fd?
#        xor %edx, %edx
#        mov $255, %ebx
#        div %ebx
#        inc %edx                        #cica facem %255 + 1
#        movb %dl, fd
#et_dim_pro_fis:
#        mov dim, %eax                   #dim e in octeti, trebuie impartita la 1024, eventual plusata cu 1
#        xor %edx, %edx
#        mov $1024, %ebx
#        div %ebx
#        mov %ebx, dim                   #dim in blocuri, daca nu e conforma vezi tu ce faci

#	jmp et_procesare_fis_exit

#et_error_procesare_fis:
#	push $formatError1
#        call printf
#        add $4, %esp
#	jmp et_procesare_fis_exit
#et_procesare_fis_exit:
#	pop %ebp
#	ret
...
#	xor %ecx, %ecx
#et_loop_concatenare_1:
#	movb (%eax), %cl   		 # Încarcă un caracter din src1 (eax) în cl
#	movb %cl, (%edx)    		# Mută caracterul din cl în dest (edx)
#	inc %eax           		 # Incrementăm adresa sursei (src1)
#	inc %edx           		 # Incrementăm adresa destinației
#	test %cl, %cl       		# Verificăm dacă caracterul este '\0'
#	jnz et_loop_concatenare_1	# Dacă nu este '\0', continuă bucla

#	mov %ebx, %eax			#adresa lui /
#et_loop_concatenare_2:
#	movb (%eax), %cl  		  # Încarcă un caracter din src2 (eax) în cl
#	movb %cl, (%edx)  		  # Mută caracterul din cl în dest (edx)
#	inc %eax          		  # Incrementăm adresa sursei (src2)
#	inc %edx          		  # Incrementăm adresa destinației
#	test %cl, %cl     		  # Verificăm dacă caracterul este '\0'
#	jnz et_loop_concatenare_2	  # Dacă nu este '\0', continuă bucla

#	mov $0, (%edx)			#\0 la final
#concat_exit:
#	pop %ebp
#	ret
