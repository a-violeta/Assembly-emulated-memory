...
show: #n am mai folosit deloc functia				#iterare cu ecx, retinere fd in edx, apelare get pt start si end
	push %ebp
	mov %esp, %ebp
	mov $memorie, %esi
	xor %ecx, %ecx
...
redimensionare:
	...
	jle redim_exit
        ...
        xor %edx, %edx
        mov $8, %ebx		#avem nevoie la impartire, pot imparti la %bl dar reg ebx tot e ocupat deci e tot aia
	div %ebx
...
et_afisare_ADD:
                                #pop %edx
        cmp dim, %eax
        jl et_spatiu_insuficient_ADD        #daca n am gasit destule poz cat are dim (ebx)
...
#et_ADD_fara_afisare: #nu e necesara, defrag trebuie sa afiseze exact la fel
#	mov %ecx, %ebx		#ult poz nula
#	sub dim, %ecx
#	inc %ecx
...
et_adaugarea_ADD:                   #se ajunge aici doar daca avem ceva valid, eax are poz start, ebx are end, dl are v[i]
        inc %ebx                #ca sa iau toate valorile fara sa modific for ul
...
et_afisare_DEL:				#un else la compararea bl si bh
	...
	movb %bh, fd
					#xor %bh, %bh
	push %ebx			#pastrez fd de sters in bl, si fd ul curent in bh

	push fd
	call GET			#am start si end
	add $4, %esp
...
DEFRAGMENTATION:
        ...
        xor %ecx, %ecx
        xor %edx, %edx
        movb $1, defrag                 #movb $0, verificare
        mov $memorie, %esi
et_iterare_DEF:
	cmp $1024, %ecx                   #DIMENSIUNE
        je et_exit_DEF
        ...
        je et_inc_DEF
                                ...
                                #je et_dim
                                #movb $1, verificare
        push %ecx                       #sa nu pierd indexarea vectorului
